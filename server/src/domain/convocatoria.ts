import { demotionCount, mercyCount, waitCounter } from './points.js';
import type {
  Contender,
  ConvocatoriaEntry,
  ConvocatoriaResult,
...
/** Per-player exclusion history for the season, in chronological order. */
export type History = Map<number, ExclusionKind[]>;
...
/**
 * Build the convocatoria for a game.
 *
 ...
 */
export function buildConvocatoria(
  signups: Contender[],
  history: History,
  rules: SeasonRules,
): ConvocatoriaResult {
  const historyOf = (id: number): ExclusionKind[] => history.get(id) ?? [];
  ...
  const ranked: Ranked[] = [...signups]
    ...
    .map((c, index) => ({
      ...c,
      index,
      wait: waitCounter(historyOf(c.playerId), rules),
      mercies: mercyCount(historyOf(c.playerId)),
      demotions: demotionCount(historyOf(c.playerId)),
    }));
  ...
  if (oversubscribed && rules.mercySeats > 0) {
    const promotees = selectPromotees(order, rules);
    const demotees = selectDemotees(order, rules, promotees.length);
...
function selectPromotees(order: Ranked[], rules: SeasonRules): Ranked[] {
  return order
    .slice(rules.slots)
 ...
 */
function selectDemotees(order: Ranked[], rules: SeasonRules, needed: number): Ranked[] {
